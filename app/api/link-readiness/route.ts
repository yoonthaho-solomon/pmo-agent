...
async function checkColumn(supabase: SupabaseClient, key: string, label: string, table: string, column: string): Promise<ReadinessCheck> {
  ...
  const missing = error.code === 'PGRST204' || error.message.includes('Could not find') || error.message.includes('does not exist')
  return {
    key,
    label,
    table,
...
export async function GET() {
  ...
  const canLinkWithoutSchemaChange = checks.some((item) => item.key === 'driver_vehicle_map' && item.status === 'ok')
  ...
  return NextResponse.json({
    ...
    conclusion: canLinkWithoutSchemaChange
      ? '현재 스키마 안에서 기사-앱미터 연결을 시도할 수 있는 키가 있습니다.'
      : '현재 스키마에는 앱미터 driver_key를 기존 driver_id로 연결할 보존 키가 부족합니다.',
    minimumPlan: [
